...
Figure5 = figure; PlotSignalHeatmap(Data=NMTable, Signal="Effector");
...
function PlotSignalHeatmap(Args)
    arguments
        Args.Data table
        Args.Signal string {mustBeMember(Args.Signal, ["Signal1", "Signal2", "Effector"])}
    end
    
    MRange = sort(unique(Args.Data.MassTransfer));
    XLabels = string(MRange);
    XLabels(mod(MRange, 1) ~= 0) = "";
    NRange = sort(unique(Args.Data.Density));
    YLabels = string(NRange);
    YLabels(mod(NRange, 0.2) ~= 0) = "";

    if Args.Signal == "Effector"
        ColorVariable = "Effector";
        Title = "Effector Expression Level";
    else
        ColorVariable = Args.Signal + "Star";
        Title = Args.Signal + " Equilibrium";
    end

    HMap = heatmap(Args.Data, "MassTransfer", "Density", ...
